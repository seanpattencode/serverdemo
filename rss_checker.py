...
if input("Start checking? (y/n): ").lower() == "y":
    for url in feeds:
        for i in ET.fromstring(urllib.request.urlopen(url[0]).read()).findall(".//item")[:3]: desc=html.unescape(re.sub('<[^>]+>','',i.findtext('description','')or'')).strip(); print(f"{i.find('title').text}\n  {i.find('link').text}\n  {desc or 'No description'}\n")
