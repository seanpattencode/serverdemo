...
P=os.path.dirname(os.path.abspath(__file__));db=sqlite3.connect(f"{P}/hdb4.db");db.executescript("CREATE TABLE IF NOT EXISTS c(k PRIMARY KEY,v);CREATE TABLE IF NOT EXISTS l(t,m)")
...
U=os.path.expanduser("~/.config/systemd/user");SVC="hdb4";on=lambda:os.popen(f"systemctl --user is-active {SVC}.service 2>/dev/null").read().strip()=="active"
